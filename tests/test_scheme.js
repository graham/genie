...
describe("Scheme Runtime", function() {
    ...
    it("supports conditions", function() {
        var scope = new Scheme.Scope();
        var result = scope.execute("(if (true) (setf x 1) (setf x 0))");
        expect(scope.get('x')).toEqual(1);

        result = scope.execute("(if (false) (setf x 1) (setf x 0))");
        expect(scope.get('x')).toEqual(0);
    });

    it("supports conditions with other values", function() {
        var scope = new Scheme.Scope();
        var result = scope.execute("(if (1) (setf x 1) (setf x 0))");
        expect(scope.get('x')).toEqual(1);

        result = scope.execute("(if (+ 0 1) (setf x 10) (setf x 100))");
        expect(scope.get('x')).toEqual(10);

        result = scope.execute("(if (not (+ 0 0)) (setf x 10) (setf x 100))");
        expect(scope.get('x')).toEqual(10);
    });

    it("supports non standard parens", function() {
        var scope = new Scheme.Scope();
        scope.expr_open = '[';
        scope.expr_closed = ']';
        var result = scope.execute("[+ 1 2 3 ]");        
