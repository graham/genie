...
html_files = find_file('entries/', filetypes=['(.*)\.html'])
templates = find_file('templates/', filetypes=['(.*)\.html'])

d = []
for i in html_files:
    d.append( (os.path.getmtime(i), i) )

d = sorted(d, key=operator.itemgetter(0), reverse=True)

f = open('data.txt', 'w')
f.write( json.dumps({'html_files':d, 'templates':templates}) )
f.close()
